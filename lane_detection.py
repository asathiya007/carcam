...
def _find_lane_lines(frame, lines):
    ...
    #     return None
    left_lane_line = _mark_lane_points(frame, left_lane_avgfit)
    if len(right_lane_fit) == 0:
        return None
    right_lane_avgfit = np.average(right_lane_fit, axis=0)
    # if np.any(np.isnan(right_lane_avgfit)): 
    #     return None
    right_lane_line = _mark_lane_points(frame, right_lane_avgfit)
...
def detect_lanes(frames):
    ...
    for frame in frames: 
        grayscaled_frame = _grayscale_frame(frame)
        masked_frame = _mask_frame(grayscaled_frame)
        thresholded_frame = _threshold_frame(masked_frame)
        detected_lines = cv2.HoughLinesP(thresholded_frame, 2, np.pi / 180, 100, np.array([]), minLineLength = 10, maxLineGap = 5)
        lane_lines = _find_lane_lines(frame, detected_lines)
        ...
        if lane_lines is None:
            ...
            if len(valid_lane_lines) >= 1:
                lane_lines = valid_lane_lines[-1]
        annotated_frame = _draw_lane_lines(frame, lane_lines)
